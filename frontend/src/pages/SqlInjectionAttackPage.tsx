...
setError('');
try {
const response = await axios.post('http://localhost:8000/api/challenges/vulnerable-login', { username, password });
...
<div className="w-full max-w-sm p-6 bg-gray-900 rounded-lg border border-gray-700">
    ...
    <div className="flex items-center justify-between">
      <button onClick={handleLogin} className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Ok</button>
      <Link to="/challenges" className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded">Cancel</Link>
    </div>
  </div>
</div>
);
