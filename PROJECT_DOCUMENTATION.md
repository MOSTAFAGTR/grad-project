...
3. Implement ZIP upload of projects up to 50 MB with safe extraction and regex-based static scanning with severity scoring and fix recommendations.
...
6. Support instructor quiz creation, assignment to students, and AI-assisted bulk generation with fallback to the static bank.
7. Provide student dashboards with challenge mastery visualization, quiz history, PDF export, and Red versus Blue game notifications.
...
Section 2 describes the learning model and feature inventory. Section 3 documents architecture (including data flow and trust boundaries), databases, and frontend state. Sections 4–8 cover authentication, challenges (including sandbox **`improvement_score`** and narrative attack XP), scanner, reports, and quizzes. **Section 9** is the **full learning analytics reference** (Defense Level, skill buckets, streak, dashboard XP, instructor/admin stats). Sections 10–13 cover logging, messaging, AI, and Red vs Blue. Sections 14–15 list APIs and pages. Sections 16–17 cover configuration and deployment. Sections 18–19 state limitations and security posture. **Section 20** documents **testing and verification**. **Section 21** documents **failure modes**. Appendices provide file tree, route map, glossary, source listings, and changelog.
...
**Scan:** The student uploads a ZIP via `POST /api/project/upload` (`projects.py`). The frontend stores results in `ScanContext` under `scale.scanData.{user_id}` in `localStorage`. The student runs `POST /api/project/upload` then `POST /api/project/scan`, which walks extracted files, applies `detector.py` rules, scores risk in `scorer.py`, attaches fixes in `fixer.py`, persists `ScanHistory` rows, and starts a background thread for `scan_dependencies()` in `dependency_scanner.py`.

**Attack:** For each lab, the student opens attack routes under `/challenges/{n}/attack` or tabbed routes, invoking endpoints such as `POST /api/challenges/vulnerable-login` or `POST /api/challenges/xss/comments` (`challenges.py`). Success is detected by HTTP responses and client-side success maps. The student may call `POST /api/challenges/mark-attack-complete` to record `UserProgress` without sandbox fix.

**Fix:** The student submits Python source replacing `app.py` in the challenge template via `POST /api/challenges/submit-fix*` routes. The backend runs `_verify_fix_improvement()` which executes `run_in_sandbox_detailed` twice (vulnerable baseline vs. student code) and computes `improvement_score`. On success, `mark_challenge_complete` inserts `UserProgress` and `recalculate_learning_progress` updates `user_learning_progress`.
...
| Static MCQ bank | Topics, difficulty, random sampling | Student | `quizzes.py`, `seed_db.py` |
...
| HTTP client | requests | unpinned | OSV HTTP |
...
`docker compose up --build` builds images, starts `sandbox_base` to completion, starts MySQL containers with healthchecks, then starts `backend` after `depends_on` conditions succeed. The backend runs `seed_db.py` then `uvicorn ... --reload`. On startup, `startup_event` retries `create_all` up to ten times with three-second delays until MySQL accepts connections. `_ensure_runtime_schema()` patches missing columns on `security_logs`, `user_learning_progress`, `teams`, `game_challenges`, `red_team_actions`, and `blue_team_fixes`. `seed_db.py` waits for MySQL, creates tables, seeds admin `admin@scale.edu` / `AdminPass123!` and instructor `instructor@scale.edu` / `TeachPass123!`, and inserts up to 200 quiz questions if fewer exist.
...
Routers: `auth` `/api/auth`, `quizzes` `/api/quizzes`, `challenges` `/api/challenges`, `stats` `/api/stats`, `messages` `/api/messages`, `projects` `/api` (no extra prefix), `game_challenge` `/api/challenge`, `misconfig` `/api`, `ai_mentor` `/api/ai`, `attack_simulator` `/api/attack`, `project_analyzer` `/api`, `security_logs` `/api/security`, `quiz_dynamic` `/api/quiz`, `instructor` `/api/instructor`, `report` prefix `/api/report`, `red_blue` `/api/redblue`.

Startup logs AI status via `OPENAI_API_KEY` and `SERPER_API_KEY` environment variables.
...
`/` Landing, `/login`, `/register` public. Protected shell: `/home` Dashboard, `/challenges`, `/messages`, `/scanner`, `/attack-lab`, challenge routes `/challenges/1..10/...`, `/challenges/attack-success`, `/redblue/game/:gameId`. Nested `user` only: `/quiz`, `/redblue/my-games`. Instructor: `/instructor/dashboard`, `/instructor/quiz`, `/redblue/create`. Admin: `/admin/stats`, `/admin/dashboard`, `/admin/logs`. Unknown paths redirect to `/`.
...
Projects and scan history JSON in `vuln_summary`.
...
| 6 | security-misc | Security Misconfiguration | `GET /api/admin/config` (misconfig router) | `/api/challenges/submit-fix-misc` | challenge-security-misc |
...
Tutorial → attack → fix → optional `mark-attack-complete`. Fix posts JSON `{"code": "..."}` to submit-fix routes. `_verify_fix_improvement` runs sandbox twice; success requires `after_result.success` and zero failures/errors.
...
These numbers are **not** sent to the backend, **not** added to `currentXp` on the dashboard, and **not** stored in any table. They exist solely to reinforce accomplishment after viewing an attack replay. The **authoritative** progress signals remain `UserProgress` (after a successful **fix**), quiz attempts, and `user_learning_progress`.

---

## 6. Project Scanner

Regex-based detector in `rules.py`; CSRF heuristic in `detector.py` requires POST patterns without CSRF token keywords. Scoring: High=5, Medium=3, Low=1; thresholds ≤10 Low, ≤20 Medium, else High. `fixer.py` attaches `FIX_RECOMMENDATIONS`. `vuln_summary` JSON includes findings, summary counts, debug, dependency_scan after background thread.
...
Two systems: `/api/quizzes` bank and assignments, `/api/quiz` dynamic generation from scan context.

### 8.2 Static Quiz Bank
`POST /api/quizzes/take` samples questions by topic/difficulty.

### 8.3 Instructor Management
`POST /api/quizzes/manage` CRUD; assignments store `question_ids` as comma-separated text in `QuizAssignment.question_ids` column.

### 8.4 Student Quiz Interface
Two-card layout on `StudentQuizPage` for bank vs scan-based.

### 8.5 Dynamic AI Quiz
`POST /api/quiz/generate` for scan-driven quizzes.

### 8.6 Instructor AI Quiz Generator
`POST /api/quizzes/ai-generate-and-assign` with `AIQuizAssignRequest` fields.

### 8.7 Capture-the-flag (CTF) competitive mode
...
`log_security_event` writes `security_logs` with `context_type` real vs challenge. `GET /api/security/logs` supports filters.
...
Scores are **event counts**, not weighted points. The API recomputes:

- **Red score:** Count of `RedTeamAction` rows for this `game_id` where `status` is `confirmed` or `NULL` (see `_red_score_query`).
- **Blue score:** Count of `BlueTeamFix` rows for this `game_id` where **`fixed == True`** (unittest passed; `fixed` comes from `_verify_fix_improvement`).

Stored `GameChallenge.red_score` / `blue_score` columns may be present for denormalization but **authoritative** counts in `get_redblue_game` are derived from the queries above. A blue fix increments the blue score only when the sandbox reports a full pass; partial improvement (`improvement_score` only) does **not** increment the blue score.
...
### 15.1 Frontend page catalog

The table below gives a **single-line purpose** for each routed page. Subsections **15.2+** retain per-file API extraction and state hooks for traceability.

| Route pattern | Component file | Role(s) | Purpose |
|---------------|------------------|---------|---------|
| `/` | `LandingPage.tsx` | Public | Marketing entry; navigation to login and registration. |
| `/login` | `LoginPage.tsx` | Public | Credential submission; JWT and identity stored in `sessionStorage`. |
| `/register` | `RegisterPage.tsx` | Public | New account creation against registration API. |
| `/home` | `DashboardHomePage.tsx` | user, instructor, admin | Student home: progress, mastery bars, quiz history, PDF download, Red vs Blue banner, AI status. |
| `/challenges` | `ChallengesListPage.tsx` | user, instructor, admin | Challenge grid with links to tutorial/attack/fix; progress and reset actions. |
| `/messages` | `MessagesPage.tsx` | user, instructor, admin | Internal messaging threads and compose. |
| `/scanner` | `Scanner.tsx` | user, instructor, admin | ZIP upload, static scan, findings, AI-enriched scan, dependency tab. |
| `/attack-lab` | `AttackLab.tsx` | user, instructor, admin | Attack lab hub / navigation helper. |
| `/challenges/1/*` | `SqlInjection*.tsx` | user, instructor, admin | SQL injection tutorial, attack, and fix flows. |
| `/challenges/2/*` | `Xss*.tsx` | user, instructor, admin | XSS tutorial, attack, and fix flows. |
| `/challenges/3/*` | `Csrf*.tsx` | user, instructor, admin | CSRF attack, fix, and tutorial flows. |
| `/challenges/4/:tab` | `CommandChallengePage.tsx` (+ `CommandInjection*.tsx`) | user, instructor, admin | Command injection tabbed lab (attack / fix / tutorial). |
| `/challenges/5/*` | `BrokenAuth*.tsx` | user, instructor, admin | Broken authentication lab pages. |
| `/challenges/6/*` | `SecurityMisc*.tsx` | user, instructor, admin | Security misconfiguration lab pages. |
| `/challenges/7/:tab` | `InsecureStorageChallengePage.tsx` (+ storage pages) | user, instructor, admin | Insecure storage tabbed lab. |
| `/challenges/8/:tab` | `DirectoryTraversalChallengePage.tsx` (+ traversal pages) | user, instructor, admin | Directory traversal tabbed lab. |
| `/challenges/9/:tab` | `XxeChallengePage.tsx` (+ XXE pages) | user, instructor, admin | XXE tabbed lab. |
| `/challenges/10/:tab` | `RedirectChallengePage.tsx` (+ redirect pages) | user, instructor, admin | Unvalidated redirect tabbed lab. |
| `/challenges/attack-success` | `AttackSuccessPage.tsx` | user, instructor, admin | Post-attack success; links to replay visualizer. |
| `/quiz` | `StudentQuizPage.tsx` | user | Standard bank quiz and scan-based quiz (two-card layout). |
| `/redblue/my-games` | `RedBlueMyGamesPage.tsx` | user | Student’s Red vs Blue game list and entry points. |
| `/redblue/game/:gameId` | `RedBlueGamePage.tsx` | user, instructor, admin | Live game: attack feed polling, fix console, end game (instructor). |
| `/under-construction` | `UnderConstructionPage.tsx` | user, instructor, admin | Placeholder for unfinished routes. |
| `/instructor/dashboard` | `InstructorDashboardPage.tsx` | instructor, admin | Class metrics, student progress, Red vs Blue game list. |
| `/instructor/quiz` | `InstructorQuizPage.tsx` | instructor, admin | Question bank management and AI generator tab. |
| `/redblue/create` | `RedBlueCreatePage.tsx` | instructor, admin | Create Red vs Blue game (rosters, lab id). |
| `/admin/stats` | `AdminStatsPage.tsx` | admin | Aggregate statistics visualization. |
| `/admin/dashboard` | `AdminDashboardPage.tsx` | admin | User approval, role changes, account management. |
| `/admin/logs` | `SecurityLogsPage.tsx` | admin | Filterable security event log viewer. |
| *(not in `App.tsx`)* | `HomePage.tsx`, `ScenarioPage.tsx` | — | **Unused** by current router; reserved or legacy. |

### 15.2 Per-file technical detail

The following subsections mirror repository files. Where **Purpose** was generic in earlier drafts, it now defers to **§15.1**; extracted API calls and state remain for audit.
...
**Purpose:** Summarized in [§15.1](#151-frontend-page-catalog); technical detail (API calls, state) follows.
...
**Purpose:** The src path is a direct path from the public folder
...
**Purpose:** Using the same video as placeholder, replace with XSS specific video if you have one
...
| `uest');   const [message, setMessage] = useState('');   const [isNavigating, setIsNavigating] = useState(false);     ...` |
| `const [isNavigating, setIsNavigating] = useState(false);      const navigate = useNavigate();    const markAttackSucc...` |
...
| `;   const [endLoading, setEndLoading] = useState(false);   const [endResult, setEndResult] = useState<{ red: number; ...` |
...
| `const [attackImpact, setAttackImpact] = useState('');   const [attackSubmitting, setAttackSubmitting] = useState(fals...` |
...
| `(true);   const [rbError, setRbError] = useState<string | null>(null);    useEffect(() => {     fetchData();   }, [])...` |
...
| `;   const [dumpResult, setDumpResult] = useState<any>(null);   const [error, setError] = useState('');   const naviga...` |
| `any>(null);   const [error, setError] = useState('');   const navigate = useNavigate();    const register = async () ...` |
...
Clone repository, copy `.env.example` to `.env`, run `docker compose up --build`, open `http://localhost:5173`, log in with seeded admin credentials.
...
Rotate secrets, HTTPS, rate limits, restrict Docker socket, harden DB credentials.
...
| 2 | `quiz_assignments.question_ids` stored as comma-separated text | No relational integrity; parsing bugs if IDs contain commas. | Keep IDs numeric; validate on write. | **Normalize** to `quiz_assignment_questions` (table already exists) as sole source of truth. |
...
| 7 | JWT stored in **sessionStorage** (and mirrored in `Authorization` header) | **Same-origin XSS** in the SPA can steal tokens and act as the user. | **Content Security Policy**, strict React escaping, no `dangerouslySetInnerHTML` in non-lab code; short-lived tokens (`ACCESS_TOKEN_EXPIRE_MINUTES`). | Move to **HttpOnly-only** session cookie with CSRF tokens for mutating requests; or BFF pattern. |
...
| 12 | Tutorial video placeholders in some lab UIs | Empty panels. | **Replace** with institution-hosted videos or remove placeholders. | CMS integration. |
...
| 14 | Admin analytics historically mixed **estimated** and **real** attempt counts | Past releases risked misleading charts. | Current code uses **`ChallengeState.attempts`** (see `stats.py`). | Add regression tests for admin queries (Section 20). |
...
| `misconfig.py` | Exposed admin-style endpoints | Separate router; labeled in docs | Misconfiguration discovery |
...
|----------------|---|---|---|---|---|---|-------|
| **JWT in sessionStorage** | ✓ | — | — | ✓ | — | — | Stolen token → session replay until expiry; `HttpOnly` cookie is also set but SPA reads Bearer from storage. |
...
5. **JWT in `sessionStorage`**: same-origin script can exfiltrate; **mitigate** with CSP, XSS hardening, short TTL, refresh token pattern if moving to HttpOnly.
...
| JWT | Prefer HttpOnly session cookie + CSRF for state-changing; rotate `SECRET_KEY`. |
...
**Platform codebase:** The repository does **not** ship a dedicated **pytest** (or similar) suite for FastAPI route handlers, React components, or end-to-end browser flows. Verification of correctness for the **host application** relies on **manual** exercise of critical paths during development (login, upload, scan, quiz, Red vs Blue, PDF export).

### 20.2 Recommended manual test checklist (release candidate)

Before a demo or examination, operators should verify at minimum:

| Area | Check |
|------|--------|
| Auth | Register student; instructor pending; admin approves; login; logout clears session; tab isolation (`sessionStorage`). |
| Scanner | Upload valid ZIP; **scan** returns findings; Dependencies tab populates or shows “no manifests”; refresh after background thread. |
| Labs | One successful attack + one successful fix per category (or spot-check) with sandbox pass. |
| Quizzes | Bank quiz (`POST /api/quizzes/take`) returns questions; submit attempt; instructor assignment visible to student. |
| Red vs Blue | Create game; red attack appears in poll; blue fix increments score when `fixed`; end game. |
| Admin | Security logs filter; user role change. |
| AI (optional) | `GET /api/ai/status` reflects keys; mentor returns fallback when keys absent. |

### 20.3 Coverage and CI (future work)

| Target | Suggested approach |
|--------|-------------------|
| API | **pytest** + `httpx.AsyncClient` against FastAPI `TestClient`; cover auth, RBAC 403, and one happy path per router. |
| Frontend | **Vitest** + React Testing Library for `ProtectedRoute`, `ScanContext`, and Scanner upload error states. |
| E2E | **Playwright** or Cypress for login → scan → quiz (optional). |
...
| GET | `/api/admin/config` | `misconfig.py` | Yes (admin) | See Section 14 |
...
**Total routes:** 108 (including `GET /`).
...
- **`currentXp`:** Client-only value on `DashboardHomePage`: `solvedLabs * 100 + (quizAttempts.length ? avgScorePercent : 0)`. Not persisted. See Section 9.6.
...
*End of document. Primary artifact: `PROJECT_DOCUMENTATION.md` (repository root). Revision: April 2026. Route inventory: Appendix B; full backend listings: Appendix M.*